/** Express router providing Hosp related routes
 * @module routers/Hosp
 * @requires express
...
/**
 * Route to list hosp records
 * @route {GET} /hosp/index/{fieldname}/{fieldvalue}
...
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		if(search){
			let searchFields = Hosp.searchFields();
		...
		query.order = Hosp.getOrderBy(req);
		query.attributes = Hosp.listFields();
		let page = parseInt(req.query.page) || 1;
		let limit = parseInt(req.query.limit) || 20;
		let result = await Hosp.paginate(query, page, limit);
...
/**
 * Route to view Hosp record
 * @route {GET} /hosp/view/{recid}
 * @param {array} path - Array of express paths
 * @param {callback} middleware - Express middleware.
 */
router.get(['/view/:recid'], async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = Hosp.viewFields();
...
, async function (req, res) {
	try{
		...
		//save Hosp record
		let record = await Hosp.create(modeldata);
...
/**
 * Route to get  Hosp record for edit
 * @route {GET} /hosp/edit/{recid}
 * @param {string} path - Express path
 * @param {callback} middleware - Express middleware.
 */
router.get('/edit/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = Hosp.editFields();
		let record = await Hosp.findOne(query);
		if(!record){
			return res.notFound();
		}
...
/**
 * Route to update  Hosp record
 * @route {POST} /hosp/edit/{recid}
 * @param {string} path - Express path
 * @param {callback} middleware - Express middleware.
 */
...
, async (req, res) => {
	try{
		...
		let recid = req.params.recid;
		let modeldata = req.body;
		let hidCount = await Hosp.count({where:{'hid': modeldata.hid, 'id': {[Op.ne]: recid} }});
		if(hidCount > 0){
			return res.badRequest(`${modeldata.hid} already exist.`);
		}
		let query = {};
...
/**
 * Route to delete Hosp record by table primary key
 * Multi delete supported by recid separated by comma(,)
 * @route {GET} /hosp/delete/{recid}
 * @param {array} path - Array of express paths
 * @param {callback} middleware - Express middleware.
 */
router.get('/delete/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		let records = await Hosp.findAll(query);
		records.forEach(async (record) => { 
			//perform action on each record before delete
		});
		await Hosp.destroy(query);
		return res.ok(recid);
	}
	catch(err){
		return res.serverError(err);
