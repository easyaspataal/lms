 ...
 * @route {GET} /admin/index/{fieldname}/{fieldvalue}
...
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		if(search){
			let searchFields = Admin.searchFields();
		...
		query.order = Admin.getOrderBy(req);
		query.attributes = Admin.listFields();
		let page = parseInt(req.query.page) || 1;
		let limit = parseInt(req.query.limit) || 20;
		let result = await Admin.paginate(query, page, limit);
...
router.get(['/view/:recid'], async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = Admin.viewFields();
		let record = await Admin.findOne(query);
		if(!record){
			return res.notFound();
		}
...
, async function (req, res) {
	try{
		...
		//save Admin record
		let record = await Admin.create(modeldata);
...
router.get('/edit/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = Admin.editFields();
		let record = await Admin.findOne(query);
		if(!record){
			return res.notFound();
		}
...
, async (req, res) => {
	try{
		...
		await Admin.update(modeldata, {where: where});
...
router.get('/delete/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		let records = await Admin.findAll(query);
		records.forEach(async (record) => { 
			//perform action on each record before delete
		});
		await Admin.destroy(query);
		return res.ok(recid);
	}
	catch(err){
		return res.serverError(err);
